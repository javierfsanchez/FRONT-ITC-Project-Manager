...
import { FormAdministradoresComponent } from 'src/app/Forms/form-administradores/form-administradores.component';
...
export class AdministradorComponent implements OnInit, AfterViewInit {
  ...
  public get(){
    this.api.get('Administradores').then((res)=>{
  ...
  public deleteRegistro(id: number) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            this.api.delete('Administradores', id.toString(), { activo: "I" }) // Llama al método Put para marcar el estudiante como inactivo
                .then(() => {
                    Swal.fire(
                        'Eliminado!',
  ...
  public editarRegistro(index: any){
    this.dialog.open(FormAdministradoresComponent, {
      width: '50%',
