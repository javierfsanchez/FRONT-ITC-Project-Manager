...
import { FormDocentesComponent } from 'src/app/Forms/form-docentes/form-docentes.component';
...
export class DocenteComponent implements OnInit, AfterViewInit {
  ...
  public get(){
    this.api.get('Docentes').then((res)=>{
  ...
  public deleteRegistro(id: number) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            this.api.delete('Docentes', id.toString(), { activo: "I" }) // Llama al método Put para marcar el estudiante como inactivo
                .then(() => {
                    Swal.fire(
                        'Eliminado!',
  ...
  public editarRegistro(index: any){
    this.dialog.open(FormDocentesComponent, {
      width: '50%',
